...
#define N_TRY 5
#define TIMER_SET 30
#define SLEEP_TIME 10 // 600으로 설정해야 10분을 sleep하게 되지만, 빠른 확인을 위해 10으로 설정하였습니다.
#define ENTER 10
#define TAB 9
#define DEL 127
...
// 입력값을 마스킹 하는데 유용한 conio 헤더가 리눅스에 없어서 구글링을 하여 getch만 따로 구현
int getch(void){
    int ch;
    struct termios buf;
    struct termios save;
...
void alarm_handler(int signo){
    if(signo == SIGALRM) {
        ...
        sleep(SLEEP_TIME); // 10분 동안 sleep하려면 600으로 설정해야하지만 시간이 너무 오래 걸리므로 10초로 설정했습니다.
...
void login_handler(int signo){
    ...
    if(signo == SIGUSR1){
        ...
        if(try_count <= 5){
            ...
            // 비밀번호는 마스킹하여 입력
            // 제 컴퓨터에서는 백스페이스 아스키 코드값을 8이 아닌 127(del)로 설정해야 지워졌습니다.
            // enter 키 또한 13이 아닌 10으로 설정해야 제대로 작동했습니다.
            while(1){
                ch = getch();
                if(ch == ENTER || ch == TAB){
                    inputpwd[i] = '\0';
        ...
        // reset_count 함수에서는 try_count 변수를 1로 초기화하고, longjmp를 통해 setjmp위치로 이동
            ...
            sleep(SLEEP_TIME); // 원래는 600으로 설정해야하지만 시간상 10초로 설정하였습니다.
            reset_count();
        }
    }
}
